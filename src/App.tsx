  ...
  return (
    <div className="min-h-screen relative overflow-hidden font-sans text-slate-800 selection:bg-orange-200 bg-linear-to-b from-amber-50 via-orange-100 to-rose-200">
      ...
      <div className="relative z-10 flex flex-col h-dvh w-full max-w-dvw p-2 sm:p-4 md:p-12">
        ...
        <header className={`shrink-0 transition-opacity duration-700 ease-in-out ${currentStep === 0 ? 'opacity-0 pointer-events-none' : 'opacity-100'} flex flex-wrap items-center gap-1 sm:gap-2 text-[10px] sm:text-xs md:text-base text-slate-600 font-medium tracking-wide mb-2 sm:mb-4 md:mb-6`}>
        ...
        <div className="fixed bottom-2 right-2 md:bottom-6 md:right-6 transition-all duration-300 opacity-60 hover:opacity-100 bg-white/70 backdrop-blur-md px-3 py-1.5 md:px-4 md:py-2 rounded-full shadow-lg border border-slate-200/50 flex items-center space-x-1.5 md:space-x-3 z-50 group origin-bottom-right scale-90 md:scale-100">
          <button onClick={handlePrev} disabled={currentStep === 0} className={`p-1 md:p-1.5 rounded-full transition-colors ${currentStep === 0 ? 'text-slate-300 cursor-not-allowed' : 'text-slate-600 hover:bg-orange-100 hover:text-orange-700'}`}>
            <ChevronLeft size={18} />
          </button>

          <div className="flex space-x-1 sm:space-x-1.5">
            {steps.map((step, idx) => (
              <button key={step.id} onClick={() => goToStep(idx)} className={`w-1.5 h-1.5 md:w-2 md:h-2 rounded-full transition-all duration-300 ${currentStep === idx ? 'bg-orange-500 w-3 md:w-4' : 'bg-slate-300 hover:bg-orange-300'}`} title={step.title} />
            ))}
          </div>

          <button onClick={handleNext} disabled={currentStep === steps.length - 1} className={`p-1 md:p-1.5 rounded-full transition-colors ${currentStep === steps.length - 1 ? 'text-slate-300 cursor-not-allowed' : 'text-slate-600 hover:bg-orange-100 hover:text-orange-700'}`}>
            <ChevronRight size={18} />
          </button>

          <div className="w-px h-4 md:h-5 bg-slate-300/80 mx-0.5 md:mx-1"></div>

          <button onClick={toggleFullScreen} className="p-1 md:p-1.5 text-slate-500 hover:text-orange-600 transition-colors" title="Toggle Full Screen">
            {presentationMode ? <Minimize size={14} className="md:w-4 md:h-4" /> : <Maximize size={14} className="md:w-4 md:h-4" />}
          </button>

          <div className="w-px h-4 md:h-5 bg-slate-300/80 mx-0.5 md:mx-1"></div>

          <button onClick={() => setShowAcrosticBreadcrumbs(!showAcrosticBreadcrumbs)} className={`p-1 md:p-1.5 transition-colors ${showAcrosticBreadcrumbs ? 'text-orange-600' : 'text-slate-500 hover:text-orange-600'}`} title="Toggle Acrostic Hierarchy Mapping">
